...
The `home-routes` handler in the `clojure-bot-conf.routes.home` namespace
...
The `home-page` function will in turn call the `clojure-bot-conf.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `clojure-bot-conf.handler` namespace:
...
Request middleware functions are located under the `clojure-bot-conf.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clojure-bot-conf.dev-middleware` namespace found in
